...
class Ui_AjusteFrete(QMainWindow):
    ...
    def salvaCotacaoAlterada(self):
        ...
        verifica = Banco.veridicaPedidoSalvo(str(Pedido))
        if verifica == ():
            print('entrei')
            Banco.salvaCotacaoNoPedido_TELACOTACAO(TRANSPORTADORA)
            sucesso = Pop_Up()
            sucesso.show()      
        else:
...
class VincularCotacaoManuel(QMainWindow):
    ...
    def salvaManual(self):
        ...
        numeroCotacao = widgets_cotacao_Manual.lineEdit_2.text()
        prazo = widgets_cotacao_Manual.lineEdit_5.text()
        if len(Pedido) == 6 :
